...
import { CATEGORIES, PRIORITIES, CROSS_MONTH_COLORS } from '@/lib/constants';
...
function CalendarContent() {
  ...
  const stats = useMemo(() => ({
    ...
    pending: activeTasks.filter((t) => !t.done).length,
    done: activeTasks.filter((t) => t.done).length,
    urgent: activeTasks.filter((t) => t.priorityLevel === 'urgent_important' && !t.done).length,
  }), [activeTasks]);
  ...
  const groupedTasks = useMemo(() => {
    ...
    for (const task of selectedDateTasks) {
      ...
      const groupKey = task.subCat ?? task.cat;
      const label = subCatName ?? catInfo?.name ?? task.cat;
      const color = catInfo?.color ?? '#94A3B8';
      if (!groups[groupKey]) groups[groupKey] = { label, color, tasks: [] };
    ...
    <div className="flex h-full">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected ? 'bg-accent-blue text-white' : day.isToday ? 'bg-accent-blue/10 text-accent-blue font-bold' : 'hover:bg-bg-elevated text-text-primary'
                  )}
                >
                  <span className={cn(day.isToday && !isSelected && 'text-accent-blue font-bold')}>{day.day}</span>
      ...
      <div className="flex w-[360px] shrink-0 flex-col border-l border-border-micro">
        <div className="flex items-center justify-between border-b border-border-micro px-4 py-3">
          ...
          <button onClick={() => setShowAddModal(true)} className="flex h-7 w-7 items-center justify-center rounded-lg gradient-brand text-white">
                ...
                {groupedTasks.map((group) => (
                  <div key={group.label}>
                    <div className="mb-1.5 flex items-center gap-2 px-2">
                      ...
                      <span className="text-[11px] text-text-muted">{group.tasks.length}项</span>
                    </div>
                    {group.tasks.map((task) => (
                      <TaskItem key={task.id} task={task} onToggleDone={(id) => updateTask(id, { done: !tasks.find(t => t.id === id)?.done })} onDelete={(id) => deleteTask(id)} onEdit={(id) => setEditingTask(tasks.find(t => t.id === id) ?? null)} />
                    ))}
                  </div>
                ))}
              </div>
